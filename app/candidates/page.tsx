...
function CandidateForm({
  ...
  return (
    <div className="p-6 space-y-4">
      ...
      {/* 面談議事録 */}
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-1">面談議事録</label>
        <p className="text-xs text-gray-400 mb-1">面談全文を記録しておくと、他の案件への提案時に活用できます</p>
        <textarea name="interviewNote" value={(form as Record<string, string>).interviewNote ?? ''} onChange={onChange} rows={10}
          placeholder="面談の内容をそのまま貼り付けてください..."
          className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" />
      </div>
              ...
              {filteredCandidates.map((candidate) => (
                <tr key={candidate.id} className="border-b border-gray-100 hover:bg-gray-50">
                  <td className="px-6 py-4">
                    ...
                    <div className="text-xs text-gray-500 mt-0.5">{candidate.type}</div>
                    <button
                      type="button"
